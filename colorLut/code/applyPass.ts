...
export class ApplyPass extends Initializable {
    ...
    protected _invalidate: (f: boolean) => void;
    ...
    public set image(file: string | File) {
        if (typeof (file) === 'string') {
            this._image.fetch(file)
                .then(() => this._invalidate(true));
        } else {
            readFile(file, (d) => this._image.fetch(d)
                .then(() => this._invalidate(true)));
        }
    }
     ...
     */
    public set invalidate(cb: (f: boolean) => void) {
        this._invalidate = cb;
    }
}
