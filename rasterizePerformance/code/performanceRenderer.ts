...
export class PerformanceRenderer extends Renderer {
    ...
    protected onFrame(): void {
        ...
        if (this._active) {
            ...
            if (!this._active) {
                ...
                let csvString =
                    'warmupFrames,frames,instances,resolution,vertices,' +
                    'time,t/res,overdraw\n';

                this._done.forEach((b) => {
                    const res = this._resolutions[b.resId];
                    const verts = this._vertexCounts[b.countId];
                    const time = b.end-b.start;
                    csvString += this._warmup + ',';
                    csvString += this._frames + ',';
                    csvString += this._instances + ',';
                    csvString += res + ',';
                    csvString += verts + ',';
                    csvString += time.toFixed(1) + ',';
                    csvString += (time / res / res).toFixed(5) + ',';
                    // https://en.wikipedia.org/wiki/Regular_polygon#Area
                    // Area when apothem a = 1
                    // Relative to incircle area
                    csvString += (
                        verts / Math.PI * Math.tan(Math.PI / verts)
                    ).toFixed(5) + '\n';
                });
    ...
    /**
     * Send new date to the graph pass.
     */
    ...
    public set numVertices(num: number) {
        this._benchmarkPass.numOuterVertices = num;
        this._invalidate(true);
    }
    ...
    public set mode(mode: string) {
        this._mode = Object.values(Mode).find((k) => k === mode);
        this.invalidate(true);
    }
}
